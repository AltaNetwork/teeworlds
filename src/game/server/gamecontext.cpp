				...
				if(str_comp_num("/w ", pMsg->m_pMessage, 3) == 0)
{
		...
		else if(MsgID == NETMSGTYPE_CL_CALLVOTE)
		{
			...
			if(str_comp_nocase(pMsg->m_Type, "option") == 0 && pPlayer->OnCallVote(pMsg->m_Value, pMsg->m_Reason))
			    return;
		...
		else if (MsgID == NETMSGTYPE_CL_SETTEAM && !m_World.m_Paused)
		{
			...
			{
				SendBroadcast_VL(_("Only {str:ID} active players are allowed"), ClientID, "ID", "69 nigga?");
			}
		}
