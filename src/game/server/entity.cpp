...
{
	return roundbyteeworlds(CheckPos.x)/32 < -200 || roundbyteeworlds(CheckPos.x)/32 > GameServer()->Collision()->GetWidth()+200 ||
			roundbyteeworlds(CheckPos.y)/32 < -200 || roundbyteeworlds(CheckPos.y)/32 > GameServer()->Collision()->GetHeight()+200 ? true : false;
}
