...
void *CEventHandler::Create(int Type, int Size, int WTeam, int64_t Mask)
{
...
void CEventHandler::Snap(int SnappingClient)
{
	...
	{
		if(SnappingClient == -1 || m_WTeam[i] < 0 || GameServer()->m_apPlayers[SnappingClient]->m_WTeam < 0 || GameServer()->m_apPlayers[SnappingClient]->m_WTeam == m_WTeam[i])
		{
